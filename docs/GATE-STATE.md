...
a reason to re-import or edit the authenticated `baseline/` bytes.
...
`baseline/` is excluded from the digest scan. It is authenticated and may only be
repaired by re-importing the same archive, so a misattribution found there would
leave the gate red with no permitted repair. The exclusion is itself tested: the
same text outside that tree must still be caught.

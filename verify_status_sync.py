...
# basename 'r1a_ffs_out_v2.c' is a prefix of the preserved
# 'r1a_ffs_out_v2.c.pre-holder'. A path token must resolve either as an exact
# repository-relative path or relative to the directory of the citing document.
...
def check_producer_attribution(root: Path) -> list[str]:
    ...
    def cites(owner: str, tokens: set[str], citing_document: str) -> bool:
        """True only when a token resolves to ``owner``.

        A token can name the producer in either of two unambiguous ways:

        * exact repository-relative path, e.g. ``r1a-device/legacy/...``;
        * a path relative to the directory containing the citing document.

        Suffix matching is deliberately forbidden. Without document-relative
        resolution, ``legacy/r1a_ffs_out_v2.c.pre-holder`` written in
        ``docs/DRIFT.md`` would falsely satisfy the owner even though it resolves
        to ``docs/legacy/...`` rather than ``r1a-device/legacy/...``.
        """
        citing_dir = posixpath.dirname(citing_document)
        owner_norm = posixpath.normpath(owner)
        ...
        for token in tokens:
            ...
            # Leading slash is treated as repository-root notation, not a host
            # filesystem path. Markdown evidence must remain repository-local.
            repository_form = posixpath.normpath(token.lstrip('/'))
            if repository_form == owner_norm:
                return True

            relative_form = posixpath.normpath(
                posixpath.join(citing_dir, token)
            )
            if relative_form == owner_norm:
                return True
    ...
    for md in sorted(root.glob('**/*.md')):
        ...
        for idx, line in enumerate(lines):
            for digest in SHA256_RE.findall(line.lower()):
                ...
                if cites(owner, tokens, rel_md):
                    continue
                wrong = [
                    other
                    for other in digest_to_path.values()
                    if other != owner and cites(other, tokens, rel_md)
                ]
